...
void main()
{
	...
	result += texture(s_Tex, vec2(inUV.x, inUV.y - u_PixelSize * 4)).rgb * weight[4];
	result += texture(s_Tex, vec2(inUV.x, inUV.y - u_PixelSize * 3)).rgb * weight[3];
	result += texture(s_Tex, vec2(inUV.x, inUV.y - u_PixelSize * 2)).rgb * weight[2];
	result += texture(s_Tex, vec2(inUV.x, inUV.y - u_PixelSize    )).rgb * weight[1];
	result += texture(s_Tex, vec2(inUV.x, inUV.y + u_PixelSize    )).rgb * weight[1];
	result += texture(s_Tex, vec2(inUV.x, inUV.y + u_PixelSize * 2)).rgb * weight[2];
	result += texture(s_Tex, vec2(inUV.x, inUV.y + u_PixelSize * 3)).rgb * weight[3];
	result += texture(s_Tex, vec2(inUV.x, inUV.y + u_PixelSize * 4)).rgb * weight[4];
